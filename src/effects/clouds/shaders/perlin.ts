export default /* glsl */ `
// Gradient noise by iq (tileable), trimmed for brevity
...
float perlinFbm(vec3 p, float freq, int octaves) {
  float G = exp2(-.85); float amp=1.; float noise=0.;
  for (int i=0;i<octaves;++i){ noise+=amp*perlinNoise(p*freq,freq); freq*=2.; amp*=G; }
  float result = noise; result = mix(1.0, result, 0.5);
  return abs(result*2.-1.);
}
float perlinFbm(vec3 p, float freq) { return perlinFbm(p, freq, 2); }
